...
export class MirrorNodeClient {
  ...
  async getNftDetails(tokenId: string): Promise<any> {
    ...
    if (!nftDetailsResponse.ok) {
      throw new Error(`Failed to fetch NFT details for token ${tokenId}`);
    }
  
    const nftDetailsJson = await nftDetailsResponse.json();
    return nftDetailsJson;  // Contains metadata and other details
  }
